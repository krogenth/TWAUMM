...
namespace TWAUMM.Draw
{
    ...
    {
        private static void DrawTribesMap(Image img, Dictionary<UInt64, Tribe> tribes, Rgba32 borderColor, float alpha = 0.0f, bool drawSidebar = true)
        {
            ...
            foreach (KeyValuePair<UInt64, Tribe> tribe in tribes)
            {
                ...
                if (drawSidebar)
                {
                    Common.DrawImageTopInformation(img, tribe.Value.name, tribe.Value.totalPoints, tribe.Value.villageCount, tribe.Key - 1, color);
                }
            }
        ...
        private static void DrawTopTribesMap(string world, Dictionary<UInt64, Tribe> tribes, string mapName, string filename)
        {
            Image img = new Image<Rgba32>(1250, 1030);
            var configInfo = Config.GetInstance().GetConfigInfo();
            DrawTribesMap(img, tribes, Common.charcoalColor, Common.noAlpha, true);
        ...
        {
            DrawTopTribesMap(world, Tribes.Tribes.GetInstance().GetTopTribes(), "Top Tribes Map", "topTribes");
        }

        public static void DrawTopODTribes(string world)
        {
            DrawTopTribesMap(world, Tribes.Tribes.GetInstance().GetTopODTribes(), "Top OD Tribes Map", "topODTribes");
        }

        public static void DrawTopODATribes(string world)
        {
            DrawTopTribesMap(world, Tribes.Tribes.GetInstance().GetTopODATribes(), "Top ODA Tribes Map", "topODATribes");
        }

        public static void DrawTopODDTribes(string world)
        {
            DrawTopTribesMap(world, Tribes.Tribes.GetInstance().GetTopODDTribes(), "Top ODD Tribes Map", "topODDTribes");
        }

        public static void DrawTopConquerTribes(string world)
        {
            Image img = new Image<Rgba32>(1250, 1030);
            DrawTribesMap(img, Tribes.Tribes.GetInstance().GetTopConquerTribes(), Common.alphaCharcoalColor, Common.alpha, false);
            ...
            foreach (var tribe in Tribes.Tribes.GetInstance().GetTopConquerTribes())
            {
                ...
                Common.DrawImageTopInformation(img, tribe.Value.name, tribe.Value.conquerPoints, (UInt64)tribe.Value.conquers.Count, tribe.Key - 1, color);
            }
        ...
        {
            Image img = new Image<Rgba32>(1250, 1030);
            DrawTribesMap(img, Tribes.Tribes.GetInstance().GetTopLossTribes(), Common.alphaCharcoalColor, Common.alpha, false);
            ...
            foreach (var tribe in Tribes.Tribes.GetInstance().GetTopLossTribes())
            {
                ...
                Common.DrawImageTopInformation(img, tribe.Value.name, tribe.Value.lossPoints, (UInt64)tribe.Value.losses.Count, tribe.Key - 1, color);
            }
